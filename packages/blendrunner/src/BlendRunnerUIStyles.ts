...
const px = function(value: number): string {
    return `${value}px`;
};

const pct = function(value: number): string {
    return `${value}%`;
};
...
export class BlendRunnerUIStyles {
    ...
    protected renderCards() {
        ...
        var cardHeader = card.child('.x-card-header', {
        ...
        var ui = test.child('.x-ui');
        var assert = test.child('.x-result').child('.x-assert', {
        ...
        var failedTest = test.and('.x-fail').child('.x-ui');
        ...
        var passedTest = test.and('.x-pass').child('.x-ui');
    ...
    protected renderHeader() {
        var me = this;
        var header = me.sheet.rule('.x-header', {
            position: 'fixed',
            width: pct(100),
            height: 'auto',
            top: px(0),
        ...
        var toolbar = header.child('.x-toolbar', {
            width: pct(100),
            height: rem(64),
        ...
        var pctbar = header.child('.x-pctbar', {
            width: pct(100),
            height: rem(8),
